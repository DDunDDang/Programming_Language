...
export default function AlignTest() {
  ...
  const justifies = ['jestify-start', 'justify-center', 'justify-end', 'justify-between', 'justify-around', 'justify-evenly'].map(justify => (
  ...
  const items = ['items-start', 'items-center', 'items-end'].map(item => (
    <div key={item} className='p-2 ml-4'>
      ...
      <div className={`flex felex-row ${item} h-20 bg-gray-300`}>{boxes}</div>
    </div>
  ))
